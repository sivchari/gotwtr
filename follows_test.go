...
func Test_deleteFollowing(t *testing.T) {
	...
	tests := []struct {
		name    string
		args    args
		want    *gotwtr.DeleteFollowingResponse
		wantErr bool
	}{
		{
			name: "200 ok",
	...
	for i, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			c := gotwtr.New("test-key", gotwtr.WithHTTPClient(tt.args.client))
			got, err := c.DeleteFollowing(tt.args.ctx, tt.args.suid, tt.args.tuid)
			if (err != nil) != tt.wantErr {
				t.Errorf("DeleteFollowing() index = %v error = %v, wantErr %v", i, err, tt.wantErr)
				return
			}
			if diff := cmp.Diff(got, tt.want); diff != "" {
				t.Errorf("DeleteFollowing() index = %v mismatch (-want +got):\n%s", i, diff)
				return
			}
		})
	}
